function complexmatrixplot(complex_data,varargin)
%   根据复数矩阵绘制色块图，用丰富的颜色和形状形象的展示矩阵元素值的大小。
%
...
if strncmpi(DisplayOpt,'On',2)
    str = num2str(data,'%4.4f');
    full_comp_data=reshape(complex_data,1,[]);
    scale = 0.07*max(n/m,1)/(max(m,n)^0.55);
    if strncmpi(TextColor,'Auto',3)
        ...
        for i = 1:numel(data)
            text(sx(i),sy(i)-0.3,0.1,str1(i,:),...
                'FontUnits','normalized',...
                'FontSize',scale,...
                'fontweight','bold',...
                'HorizontalAlignment','center',...
                'Color',TextColor(i,:));
            text(sx(i),sy(i)+0.3,0.1,str2(i,:),...
                'FontUnits','normalized',...
                'FontSize',scale,...
                'fontweight','bold',...
                'HorizontalAlignment','center',...
...
%         text(sx,sy,0.1*ones(size(sx)),str1,...
%             'FontUnits','normalized',...
%             'FontSize',scale,...
%             'fontweight','bold',...
%             'HorizontalAlignment','center',...
...
% disp(sy)
        for i = 1:numel(data)
            strnew={[num2str(real(full_comp_data(i)),'%4.4f'),'+'],[num2str(imag(full_comp_data(i)),'%4.4f'),'i']};
            text(sx(i),sy(i),0.1,strnew,...
                'FontUnits','normalized',...
                'FontSize',scale,...
                'fontweight','bold',...
                'HorizontalAlignment','center');
